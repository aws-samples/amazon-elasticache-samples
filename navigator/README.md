# ElastiCache Navigator


ElastiCache Navigator aims to address the need for unified observability and diagnostics capabilities in ElastiCache clusters 
running Valkey & Redis OSS. As customers migrate from Redis OSS to Valkey, they can use ElastiCache Navigator as a 
...
* **CLI Interface** to interact directly in ElastiCacheNavigator as a unified tool
* **Command Logs** such as slow commands, large requests and large replies


---
### ElastiCache Navigator provides an easy interface thru the browser :
![ApplicationOverview](help/ElastiCacheNavigatorApp.png) 
---

## Overview

This application contains of 3 parts
* Frontend - React application - UI for ElastiCache Navigator
...
ElastiCacheNavigator is built as a Docker Application so it can be deployed in many environments. This includes local desktops, servers or other services like Amazon EKS. Here is how it looks like in a local Desktop environment:

Here is an example on how ElastiCacheNavigator is deployed in Docker Desktop. Visible are the two containers for frontend and backend linked together as docker composer.
...
For the first time after cloning, the following commands should be run in the `navigator` folder:

```
$ cd your_path/navigator
...
If you modify some code afterwards, just run the following shell script from the `navigator` folder
...
Once docker containers are started and you connect with the browser, the ElastiCache Navigator will have a Help & Documentation section online. 
